  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-gray-900 flex items-center"><BarChart3 className="mr-3 text-blue-600" /> Balance de Ventas</h2>
        <select value={filtro} onChange={e=>setFiltro(e.target.value)} className="bg-white border text-gray-700 px-4 py-2 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none font-semibold shadow-sm">
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mt-8">
            <h3 className="text-lg font-bold text-gray-800 mb-4">Registro Histórico ({filtro})</h3>
                ...
                <tbody className="divide-y divide-gray-100">
                  {ventas.map(v => (
                    <tr key={v.id} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="p-4 text-right font-bold text-green-600">${v.total.toFixed(2)}</td>
                    </tr>
                  ))}
                  {ventas.length === 0 && <tr><td colSpan="4" className="p-8 text-center text-gray-400">Sin ventas en este periodo.</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
